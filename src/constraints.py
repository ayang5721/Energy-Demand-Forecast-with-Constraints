...
def greedy_dispatch(demand_mw: float, generator_fleet: pd.DataFrame) -> dict:
    ...
    for row in ordered_fleet.itertuples(index=False):
        generation_mw = min(float(row.max_mw), left)
        generation_mw = max(0.0, generation_mw)
        left -= generation_mw
        total_mw += generation_mw
        cost += generation_mw * float(row.cost_per_mwh)
        result[f"{row.generator}_generation_mw"] = generation_mw

    unmet_demand_mw = max(0.0, left)
...
def _dispatch_many(demand_mw, generator_fleet: pd.DataFrame) -> dict:
    ...
    left = demand.copy()
    gen_total = np.zeros_like(demand)
    cost = np.zeros_like(demand)
    result = {}

    ordered_fleet = generator_fleet.sort_values(["cost_per_mwh", "dispatch_order"])
    for row in ordered_fleet.itertuples(index=False):
        generation_mw = np.minimum(float(row.max_mw), left)
        generation_mw = np.maximum(0.0, generation_mw)
        left -= generation_mw
        gen_total += generation_mw
        cost += generation_mw * float(row.cost_per_mwh)
        result[f"{row.generator}_generation_mw"] = generation_mw

    unmet_demand_mw = np.maximum(0.0, left)
    feasible = unmet_demand_mw <= DISPATCH_TOLERANCE
    result.update(
        {
...
def run_constrained_dispatch(zone_predictions_df: pd.DataFrame, generator_fleet: pd.DataFrame) -> pd.DataFrame:
    ...
    scheduled = _dispatch_many(out["predicted_zone_load_mw"], generator_fleet)
    oracle = _dispatch_many(out["true_zone_load_mw"], generator_fleet)

    out["scheduled_generation_mw"] = scheduled["total_generation_mw"]
    out["base_generator_cost"] = scheduled["dispatch_cost"]
    out["dispatch_cost"] = out["base_generator_cost"]
    out["oracle_generation_mw"] = oracle["total_generation_mw"]
    out["oracle_base_generator_cost"] = oracle["dispatch_cost"]
    ...
    out["feasible_for_forecast"] = scheduled["feasible"]
    out["unmet_forecast_demand_mw"] = scheduled["unmet_demand_mw"]
    ...
    for generator in generator_fleet["generator"]:
        ...
        out[column] = scheduled.get(column, 0.0)
        out[f"oracle_{column}"] = oracle.get(column, 0.0)
...
def make_post_constraint_metrics(dispatch_df: pd.DataFrame) -> pd.DataFrame:
    def metrics(group: pd.DataFrame) -> pd.Series:
        ...
        base_cost = group["base_generator_cost"].sum()
        penalty_cost = group["penalty_cost"].sum()
        total_cost = group["total_operational_cost"].sum()
        oracle_cost = group["oracle_total_operational_cost"].sum()
        regret = group["constraint_regret"].sum()
        return pd.Series(
            {
                "n_hours": len(group),
                ...
                "total_base_generator_cost": base_cost,
                ...
                "oracle_total_operational_cost": oracle_cost,
                "total_constraint_regret": regret,
                "mean_constraint_regret": group["constraint_regret"].mean(),
                "total_dispatch_cost": base_cost,
